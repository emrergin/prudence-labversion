<template>
  <div v-if="question === 'generalrisk'">
    <h4>
      Genel olarak, risk almaya istekli bir kişi mi yoksa risk almaktan sakınan
      biri misiniz?
    </h4>
    <p class="m-bot">
      Lütfen cevabınızı 0'dan 10'a kadar bir ölçekte belirtin. Burada 0, "risk
      almaya tamamen isteksiz" ve 10, "risk almaya çok istekli" anlamına gelir.
      Ölçekte nereye düştüğünüzü belirtmek için 0 ile 10 arasında herhangi bir
  ...
  <div v-if="question === 'willingnesstoact'">
    <div>
      ...
      <p>
        Lütfen cevabınızı 0'dan 10'a kadar bir ölçekte tekrar belirtin. 0,
        “hiçbir şekilde yapmak istemiyor” ve 10, “yapmaya çok istekli” anlamına
    ...
    <label class="statement"
      >Gelecekte daha fazla faydasını görebilmek adına bugün sizin için faydalı
      olan bir şeyden vazgeçmeye ne kadar isteklisiniz?</label
    >
    <ul class="likert">
      <li>
    ...
    <label class="statement"
      >Size maliyetleri olsa bile, size karşı haksız davranışları cezalandırmaya
      ne kadar isteklisiniz?</label
    >
    <ul class="likert">
      <li>
    ...
    <label class="statement"
      >Size maliyetleri olsa bile, başkalarına haksızlık yapan birini
      cezalandırmaya ne kadar isteklisiniz?</label
    >
    <ul class="likert">
      <li>
    ...
    <label class="statement"
      >Karşılığında hiçbir geri dönüş beklemeden hayır işlerine bir şeyler
      vermeye ne kadar isteklisiniz?</label
    >
    <ul class="likert">
      <li>
  ...
  <div v-if="question === 'describe'" class="minusmargin">
    <div>
      ...
      <label class="statement smalltoppadding"
        >Bana iyilikte bulunan birisine karşılık vermeye hazırım.</label
      >
      <ul class="likert">
        <li>
      ...
      <label class="statement smalltoppadding"
        >Eğer çok haksız bir muamele görürsem, bunu yapmak için bir bedel olsa
        bile ilk fırsatta bunun intikamını alırım.</label
      >
      <ul class="likert">
        <li>
      ...
      <label class="statement smalltoppadding"
        >Ben insanların sadece en iyi niyetlerle davrandıklarını
        varsayarım.</label
      >
      <ul class="likert">
        <li>
      ...
      <label class="statement smalltoppadding"
        >Görevleri hemen yapmanın daha iyi olacağını bilsem bile erteleme
        eğilimindeyim.</label
      >
      <ul class="likert">
        <li>
  ...
  <div v-if="question === 'stairrisk'">
    <div class="subdiv">
      ...
      <p>
        {{ 150 * inflationMultiplier }} TL kazanma ve hiçbir şey kazanamama
        arasında eşit şansa sahip olacağınız bir çekiliş yahut belirli bir
        miktar kesin ödeme arasında seçim yapabilirsiniz. Size böyle beş farklı
        durum sunacağız.
      </p>
    </div>
    <div class="u-bot">
      <p>Hangisini tercih ederdiniz:</p>
      <p id="stairStep" class="stair-step">
        %50 şansla
        <strong>{{ inflationMultiplier * stair1SureOutcome }} TL</strong> para
        kazandıran ve %50 şansla hiçbir şey kazandırmayan bir çekilişi mi yahut
        <strong>{{ currentSure * inflationMultiplier }} TL</strong>’lik kesin
        bir nakit para ödemesini mi?
      </p>
      <input
        type="radio"
        id="stairRiskA"
        name="stairRisk"
        v-model="currentStairValue"
        value="1"
        required
      />
  ...
  <div v-if="question === 'gift'">
    <div class="subdiv">
      ...
      <p>
        ...
        {{ 8 * inflationMultiplier }} TL’ye mal olacaktır. Buna rağmen bu
        yabancı sizden hiç para istememektedir. Yanınızda 6 adet hediye var. En
        ucuz hediye {{ 2 * inflationMultiplier }} TL, en pahalı olanı ise
        {{ 12 * inflationMultiplier }} TL değerinde.
      </p>

      <p>
        Bu hediyelerden birini “teşekkür hediyesi” olarak bu yabancıya verir
        misiniz?
      </p>
    </div>
    <div>
    ...
    <div class="subdiv" v-if="currentGiftSelection === 'next'">
      ...
      <ul class="likert sixmemberlikert">
        <li>
          ...
          <label for="g2"
            >{{ 2 * inflationMultiplier }} TL değerinde hediye</label
          >
        </li>
        <li>
          <input id="g4" type="radio" name="giftvalue" value="4" />
          <label for="g4"
            >{{ 4 * inflationMultiplier }} TL değerinde hediye</label
          >
        </li>
        <li>
          <input id="g6" type="radio" name="giftvalue" value="6" />
          <label for="g6"
            >{{ 6 * inflationMultiplier }} TL değerinde hediye</label
          >
        </li>
        <li>
          <input id="g8" type="radio" name="giftvalue" value="8" />
          <label for="g8"
            >{{ 8 * inflationMultiplier }} TL değerinde hediye</label
          >
        </li>
        <li>
          <input id="g10" type="radio" name="giftvalue" value="10" />
          <label for="g12"
            >{{ 10 * inflationMultiplier }} TL değerinde hediye</label
          >
        </li>
        <li>
          <input id="g12" type="radio" name="giftvalue" value="12" />
          <label for="g12"
            >{{ 12 * inflationMultiplier }} TL değerinde hediye</label
          >
        </li>
      </ul>
    </div>
  ...
  <div v-if="question === 'hypodonation'">
    <div class="subdiv">
      ...
      <p>
        ...
        aldınız. İyi bir amaç için bu miktarın ne kadarını bağışlarsınız? (0 ile
  ...
  <div v-if="question === 'stairpatience'">
    <div class="subdiv">
      <p>
        Size bugün bir ödeme alma veya 12 ay sonra bir ödeme alma arasında seçim
        hakkı verildiğini varsayın. Şimdi size beş durum sunacağız. Bugünkü
        ödeme miktarı, bu beş durumun her birinde aynıdır. 12 ay sonra ödenecek
        miktar ise her durumda farklıdır.
      </p>
      <p>
    ...
    <div class="u-bot">
      ...
      <p id="stairStep" class="stair-step">
        ...
        12 ay sonra
        <strong>{{ valueNow * inflationMultiplier }} TL</strong> almayı mı
        tercih edersiniz?
      </p>
      <input
        type="radio"
...
function nextQuestion() {
  ...
  if (questionList[questionIndex.value] === `stairrisk`) {
    ...
    if (
      stairRiskSelections.value[stairRiskSelections.value.length - 1] === "1"
    ) {
      nextValue =
        currentSure.value +
        (1 / 2 ** stairRiskSelections.value.length) * stair1StartingValue;
    } else {
      nextValue =
        currentSure.value -
        (1 / 2 ** stairRiskSelections.value.length) * stair1StartingValue;
  ...
  if (
    questionList[questionIndex.value] === "stairrisk" ||
    questionList[questionIndex.value] === "stairpatience"
  ) {
    if (questionList[questionIndex.value] === "stairrisk") {
      store.gps.stairrisk = stairRiskSelections.value.reduce(
        (prev, curr, index) => prev + curr * (16 / 2 ** index),
        1
      );
    } else {
      store.gps.stairpatience =
        33 -
        stairPatienceSelections.value.reduce(
          (prev, curr, index) => prev + curr * (16 / 2 ** index),
          1
        );
    }
    ...
    if (currentGiftSelection.value === "next") {
      store.gps.gift = document.querySelector(
        "input[name=giftvalue]:checked"
      ).value;
    } else {
